...
class EditAmmoScreen extends ConsumerStatefulWidget {
  const EditAmmoScreen({super.key, required this.id});

  final String id;

  @override
  ConsumerState<EditAmmoScreen> createState() => _EditAmmoScreenState();
}

class _EditAmmoScreenState extends ConsumerState<EditAmmoScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      final ammo =
          await ref.read(ammoRepositoryProvider).getById(widget.id);
      if (ammo != null && mounted) {
        setState(() {
          _brandController.text = ammo.brand;
          _productLineController.text = ammo.productLine ?? '';
          _caliberController.text = ammo.caliber;
          _grainController.text = ammo.grain?.toString() ?? '';
          _bulletTypeController.text = ammo.bulletType;
          _caseMaterialController.text = ammo.caseMaterial ?? '';
          _quantityPerBoxController.text = ammo.quantityPerBox?.toString() ?? '';
          _costPerBoxController.text =
              ammo.costPerBox?.toStringAsFixed(2) ?? '';
          _roundsOnHandController.text = ammo.roundsOnHand?.toString() ?? '';
  ...
  Future<void> _save() async {
    ...
    try {
      await ref.read(ammoRepositoryProvider).update(
            id: widget.id,
      ...
      if (mounted) {
        ref.invalidate(ammoProductByIdProvider(widget.id));
        ref.invalidate(ammoProductsProvider);
        context.pop();
      }
    } finally {
